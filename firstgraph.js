function firstgraph(callback) {
  ...
  d3.csv(defaultCsvFile).then(function (data) {
    ...
    incomeData.forEach((value, key) => {
      ...
      const avgIncome =
        d3.sum(value, (d) => +d.Income) / d3.count(value, (d) => +d.Income);
      const povertyRate =
        d3.sum(value, (d) => +d.Poverty) / d3.count(value, (d) => +d.Poverty);
      //gender population
      const Men = d3.sum(value, (d) => +d.Men) / d3.count(value, (d) => +d.Men);
      const Women =
        d3.sum(value, (d) => +d.Women) / d3.count(value, (d) => +d.Women);
      //population race distribution total of 100%
      const Hispanic =
        d3.sum(value, (d) => +d.Hispanic) / d3.count(value, (d) => +d.Hispanic);
      const White =
        d3.sum(value, (d) => +d.White) / d3.count(value, (d) => +d.White);
      const Black =
        d3.sum(value, (d) => +d.Black) / d3.count(value, (d) => +d.Black);
      const Native =
        d3.sum(value, (d) => +d.Native) / d3.count(value, (d) => +d.Native);
      const Asian =
        d3.sum(value, (d) => +d.Asian) / d3.count(value, (d) => +d.Asian);
      const Pacific =
        d3.sum(value, (d) => +d.Pacific) / d3.count(value, (d) => +d.Pacific);
      //employment distribution total of 100%
      const Professional =
        d3.sum(value, (d) => +d.Professional) /
        d3.count(value, (d) => +d.Professional);
      const Service =
        d3.sum(value, (d) => +d.Service) / d3.count(value, (d) => +d.Service);
      const Office =
        d3.sum(value, (d) => +d.Office) / d3.count(value, (d) => +d.Office);
      const Construction =
        d3.sum(value, (d) => +d.Construction) /
        d3.count(value, (d) => +d.Construction);
      const Production =
        d3.sum(value, (d) => +d.Production) /
        d3.count(value, (d) => +d.Production);
      //transportation to work distribution total of 100%
      const Drive =
        d3.sum(value, (d) => +d.Drive) / d3.count(value, (d) => +d.Drive);
      const Carpool =
        d3.sum(value, (d) => +d.Carpool) / d3.count(value, (d) => +d.Carpool);
      const Transit =
        d3.sum(value, (d) => +d.Transit) / d3.count(value, (d) => +d.Transit);
      const Walk =
        d3.sum(value, (d) => +d.Walk) / d3.count(value, (d) => +d.Walk);
      const OtherTransp =
        d3.sum(value, (d) => +d.OtherTransp) /
        d3.count(value, (d) => +d.OtherTransp);
      const WorkAtHome =
        d3.sum(value, (d) => +d.WorkAtHome) /
        d3.count(value, (d) => +d.WorkAtHome);
      //work sector distribution total of 100%
      const PrivateWork =
        d3.sum(value, (d) => +d.PrivateWork) /
        d3.count(value, (d) => +d.PrivateWork);
      const PublicWork =
        d3.sum(value, (d) => +d.PublicWork) /
        d3.count(value, (d) => +d.PublicWork);
      const SelfEmployed =
        d3.sum(value, (d) => +d.SelfEmployed) /
        d3.count(value, (d) => +d.SelfEmployed);
      const FamilyWork =
        d3.sum(value, (d) => +d.FamilyWork) /
        d3.count(value, (d) => +d.FamilyWork);
      const Unemployment =
        d3.sum(value, (d) => +d.Unemployment) /
        d3.count(value, (d) => +d.Unemployment);
      completeData.push({
        state: key,
        avgIncome: avgIncome,
      ...
      d3.csv(csvFile).then(function (data) {
        ...
        incomeData.forEach((value, key) => {
          ...
          const avgIncome =
            d3.sum(value, (d) => +d.Income) / d3.count(value, (d) => +d.Income);
          const povertyRate =
            d3.sum(value, (d) => +d.Poverty) /
            d3.count(value, (d) => +d.Poverty);
          //gender population
          const Men =
            d3.sum(value, (d) => +d.Men) / d3.count(value, (d) => +d.Men);
          const Women =
            d3.sum(value, (d) => +d.Women) / d3.count(value, (d) => +d.Women);
          //population race distribution total of 100%
          const Hispanic =
            d3.sum(value, (d) => +d.Hispanic) /
            d3.count(value, (d) => +d.Hispanic);
          const White =
            d3.sum(value, (d) => +d.White) / d3.count(value, (d) => +d.White);
          const Black =
            d3.sum(value, (d) => +d.Black) / d3.count(value, (d) => +d.Black);
          ...
          const Asian =
            d3.sum(value, (d) => +d.Asian) / d3.count(value, (d) => +d.Asian);
          const Pacific =
            d3.sum(value, (d) => +d.Pacific) /
            d3.count(value, (d) => +d.Pacific);
          //employment distribution total of 100%
          const Professional =
            d3.sum(value, (d) => +d.Professional) /
            d3.count(value, (d) => +d.Professional);
          const Service =
            d3.sum(value, (d) => +d.Service) /
            d3.count(value, (d) => +d.Service);
          const Office =
            d3.sum(value, (d) => +d.Office) / d3.count(value, (d) => +d.Office);
          const Construction =
            d3.sum(value, (d) => +d.Construction) /
          ...
          const Drive =
            d3.sum(value, (d) => +d.Drive) / d3.count(value, (d) => +d.Drive);
          const Carpool =
            d3.sum(value, (d) => +d.Carpool) /
            d3.count(value, (d) => +d.Carpool);
          const Transit =
            d3.sum(value, (d) => +d.Transit) /
            d3.count(value, (d) => +d.Transit);
          const Walk =
            d3.sum(value, (d) => +d.Walk) / d3.count(value, (d) => +d.Walk);
          const OtherTransp =
            d3.sum(value, (d) => +d.OtherTransp) /
  ...
  function displayBarGraph(data) {
    ...
    var bars = svg
      ...
      .on("mouseover", function (d) {
        ...
        tooltip
          .html(
            d.srcElement.__data__.state +
              " Income: $" +
              d.srcElement.__data__.avgIncome.toFixed(2)
          )
      ...
      .on("click", function (d) {
        ...
        if (!isSelected) {
          ...
          const selectedState = d3
            .selectAll("rect")
            .nodes()
            .map((node) => node.__data__);
          const selectedStates = selectedState.filter(
  ...
  function selectedState(state) {
    ...
    d3.selectAll("rect").classed("selected", false);
    const statePath = d3.selectAll("rect").filter(function () {
