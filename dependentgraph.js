...
function raceBarChart(data) {
    ...
    function resetBarChart(data) {
        ...
        yScale.domain([0, d3.max(completeData, d => d3.max(Object.values(d)))])
                .range([height - margin.bottom, margin.top]);
        bars.transition()
        .duration(500)
        .attr("y", d => yScale(d[1]))
...
function raceRadargraphdefault(data){
    ...
    const totalData = {
      ...
      Hispanic: d3.sum(combinedData, d => d3.sum(d, e => +e.Hispanic)) / d3.sum(combinedData, d => d.length),
      White: d3.sum(combinedData, d => d3.sum(d, e => +e.White)) / d3.sum(combinedData, d => d.length),
      Black: d3.sum(combinedData, d => d3.sum(d, e => +e.Black)) / d3.sum(combinedData, d => d.length),
      Native: d3.sum(combinedData, d => d3.sum(d, e => +e.Native)) / d3.sum(combinedData, d => d.length),
      Asian: d3.sum(combinedData, d => d3.sum(d, e => +e.Asian)) / d3.sum(combinedData, d => d.length),
      Pacific: d3.sum(combinedData, d => d3.sum(d, e => +e.Pacific)) / d3.sum(combinedData, d => d.length),
    };
    
    completeData.push(totalData);
    const attributes = ["Hispanic", "White", "Black", "Native", "Asian", "Pacific"];
    const width = 300;
    const height = 300;
    const radius = Math.min(width, height-50) / 2;

    const svg = d3.select("#raceRadar")
    .append("svg")
    .attr("width", width)
    .attr("height", height);
    ...
    function updateRadarChart (data) {
      radarLine.datum(attributes.map(attr => data[attr]))
        .transition()
        .duration(500)
        .attr("d", line)
    ...
    function resetRadarChart (data){
        ...
        completeData.push(totalData);
        radarLine.datum(attributes.map(attr => completeData[0][attr]))
        .transition()
        .duration(500)
        .attr("d", line)
        .attr("fill", colorScale(0.5))
...
function employmentRadargraphdefault(data){
    ...
    const totalData = {
      ...
      Professional: d3.sum(combinedData, d => d3.sum(d, e => +e.Professional)) / d3.sum(combinedData, d => d.length),
      Service: d3.sum(combinedData, d => d3.sum(d, e => +e.Service)) / d3.sum(combinedData, d => d.length),
      Office: d3.sum(combinedData, d => d3.sum(d, e => +e.Office)) / d3.sum(combinedData, d => d.length),
      Construction: d3.sum(combinedData, d => d3.sum(d, e => +e.Construction)) / d3.sum(combinedData, d => d.length),
      Production: d3.sum(combinedData, d => d3.sum(d, e => +e.Production)) / d3.sum(combinedData, d => d.length),
    };
    
    completeData.push(totalData);
    const attributes = ["Professional", "Service", "Office", "Construction", "Production"];
    const width = 300;
    const height = 300;
    const radius = Math.min(width, height-50) / 2;

    const svg = d3.select("#employmentRadar")
    .append("svg")
    .attr("width", width)
    .attr("height", height);
...
function transportRadargraphdefault(data){
    ...
    const totalData = {
      ...
      Drive: d3.sum(combinedData, d => d3.sum(d, e => +e.Drive)) / d3.sum(combinedData, d => d.length),
      Carpool: d3.sum(combinedData, d => d3.sum(d, e => +e.Carpool)) / d3.sum(combinedData, d => d.length),
      Transit: d3.sum(combinedData, d => d3.sum(d, e => +e.Transit)) / d3.sum(combinedData, d => d.length),
      Walk: d3.sum(combinedData, d => d3.sum(d, e => +e.Walk)) / d3.sum(combinedData, d => d.length),
      OtherTransp: d3.sum(combinedData, d => d3.sum(d, e => +e.OtherTransp)) / d3.sum(combinedData, d => d.length),
      WorkAtHome: d3.sum(combinedData, d => d3.sum(d, e => +e.WorkAtHome)) / d3.sum(combinedData, d => d.length),
    };
    
    completeData.push(totalData);
    const attributes = ["Drive", "Carpool", "Transit", "Walk", "OtherTransp", "WorkAtHome"];
    const width = 300;
    const height = 300;
    const radius = Math.min(width, height-50) / 2;

    const svg = d3.select("#transportRadar")
    .append("svg")
    .attr("width", width)
    .attr("height", height);
...
function worksectorRadargraphdefault(data){
    ...
    const totalData = {
      ...
      PrivateWork: d3.sum(combinedData, d => d3.sum(d, e => +e.PrivateWork)) / d3.sum(combinedData, d => d.length),
      PublicWork: d3.sum(combinedData, d => d3.sum(d, e => +e.PublicWork)) / d3.sum(combinedData, d => d.length),
      SelfEmployed: d3.sum(combinedData, d => d3.sum(d, e => +e.SelfEmployed)) / d3.sum(combinedData, d => d.length),
      FamilyWork: d3.sum(combinedData, d => d3.sum(d, e => +e.FamilyWork)) / d3.sum(combinedData, d => d.length),
      Unemployment: d3.sum(combinedData, d => d3.sum(d, e => +e.Unemployment)) / d3.sum(combinedData, d => d.length)
    };
    
    completeData.push(totalData);
    //console.log(completeData[0])
    const attributes = ["PrivateWork", "PublicWork", "SelfEmployed", "FamilyWork", "Unemployment"];
    const width = 300;
    const height = 300;
    const radius = Math.min(width, height-50) / 2;

    const svg = d3.select("#worksectorRadar")
    .append("svg")
    .attr("width", width)
    .attr("height", height);
