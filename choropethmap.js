function choroplethmap(aggregatedData){
    ...
    d3.json("us_state.geojson").then(function(geojson) {
      ...
      function updateTextboxContent(stateName) {
        ...
        if (stateName) {
            ...
            const statePopulationEntry = aggregatedData.find(item => item.state === stateName);
            ...
            const totalPopulation = d3.sum(aggregatedData, d => d.totalPopulation);
    ...
    .on("click", function(event, d) {
      ...
      //console.log(aggregatedData)
        ...
        if (!isSelected) {
          ...
          const selectedState = aggregatedData.filter(state => state.state === stateName);
          //console.log(selectedState)
          triggerSelectEvent(selectedState[0]);
        }else{
          const selectedStates = aggregatedData.filter(state => state !== undefined);
      ...
      .translateExtent([[-2 * width, -height], [2 * width, 2 * height]])
      .on("zoom", zoomed);
      ...
      function zoomed(event) {
        ...
        const legend = svg
          .append("g")
          .attr("class", "legend")
          .attr("transform", `translate(${width - padding.right - legendWidth}, ${height - padding.bottom - legendHeight-20})`);
        ...
        const legendAxis = d3
          .axisBottom()
          .scale(d3.scaleLinear().domain([0, d3.max(aggregatedData, d => d.totalPopulation)]).range([0, legendWidth]))
          .ticks(5);
        ...
        legendGradient
          ...
          .append("stop")
          .attr("offset", d => `${d * 100}%`)
          .attr("stop-color", d => legendColorScale(d3.max(aggregatedData, d => d.totalPopulation) * d))
          .attr("stop-opacity", 1);
  ...
  function selectedState(state) {
    ...
    d3.selectAll("path").classed("selected", false);
    const statePath = d3.selectAll("path")
    .filter(function() {
      const pathData = d3.select(this).datum(); // Access the data of the current path
      return pathData && pathData.properties && pathData.properties.NAME === state;
    });
